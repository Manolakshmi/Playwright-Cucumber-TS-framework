...
When('user search for {string}', async function (searchTerm: string) {
    ...
    const searchInput = await fixture.page.getByRole('searchbox', { name: 'Search input' });
...
When('user clicks on {string} Dropdown', async function (dropdownName: string) {
    const dropdown = await fixture.page.getByRole('button', { name: dropdownName });
    await dropdown.click();
});

When('user search for {string} in Skill Dropdown', async function (skillSearchTerm: string) {
    try {
        const skillsDropdownRegion = await fixture.page.getByRole('region', { name: 'Skill' });
...
Then('UI results should match the API results', async function (this: CustomWorld) {
